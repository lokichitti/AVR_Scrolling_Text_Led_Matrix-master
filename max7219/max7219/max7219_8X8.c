...
int swapBits(unsigned int n)
{
	...
	unsigned int bit1 = (n >> 0) & 1;

	/* Move p2'th to rightmost side */
	unsigned int bit2 = (n >> 7) & 1;
...
void initSPI(void) 
{
  DDRB |= (1 << PB0);	    // Set SS output 
  PORTB |= (1 << PB0);      // Begin high (unselected)

  DDRB |= (1 << PB5);       // Output on MOSI 
  DDRB |= (1 << PB7);       // Output on SCK 
...
{
		for(uint8_t i = 5; i >=0; i--)				// For 5 bytes representing each character
		{
			pushBuffer(swapBits(pgm_read_byte(&characters[c][i])));   // Push the byte from the characters array to the display buffer
